;; haven-traits
;; Manages on-chain NFT traits and attributes for rarity and filtering
...
(define-map token-traits
  ...
  (list 10 { 
    ...
    trait-value: (string-ascii 64) 
  })
)

(define-map collection-trait-types
  uint
...
(define-public (set-token-traits (token-id uint) (traits (list 10 { trait-type: (string-ascii 32), trait-value: (string-ascii 64) })))
  (let
    (
      (token-owner-result (unwrap! (contract-call? .haven-token get-owner token-id) ERR-TOKEN-NOT-FOUND))
      (token-owner (unwrap! token-owner-result ERR-TOKEN-NOT-FOUND))
    )
    (asserts! (or 
      (is-eq tx-sender token-owner)
      (unwrap-panic (is-authorized-setter tx-sender))
    ) ERR-NOT-AUTHORIZED)
...
(define-public (add-collection-trait-type (collection-id uint) (trait-type (string-ascii 32)))
  (let
    (
      (collection (unwrap! (contract-call? .haven-registry get-collection collection-id) ERR-TOKEN-NOT-FOUND))
