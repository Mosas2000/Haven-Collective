...
(define-map royalty-config uint {recipients: (list 5 principal), percentages: (list 5 uint)})
(define-read-only (get-royalty-config (collection-id uint))
  (ok (map-get? royalty-config collection-id)))
(define-read-only (get-default-royalty) (ok (var-get default-royalty)))
(define-read-only (calculate-royalty (collection-id uint) (sale-amount uint))
  (let ((config (map-get? royalty-config collection-id))
...
(define-public (set-royalty (collection-id uint) (recipients (list 5 principal)) (percentages (list 5 uint)))
  (let ((collection (unwrap! (contract-call? .haven-registry get-collection collection-id) ERR-COLLECTION-NOT-FOUND))
    ...
    (map-set royalty-config collection-id {recipients: recipients, percentages: percentages})
    (ok true)))
(define-public (distribute-royalty (collection-id uint) (sale-amount uint))
  (let ((config (map-get? royalty-config collection-id)))
    (if (is-some config)
      (let ((royalty-data (unwrap-panic config))
            (recipients (get recipients royalty-data))
            (percentages (get percentages royalty-data)))
        (ok (fold distribute-to-recipient
          (map create-distribution-tuple recipients percentages) sale-amount)))
      (ok sale-amount))))
...
(define-private (distribute-to-recipient (dist-tuple {recipient: principal, percentage: uint}) (remaining-amount uint))
  (let ((amount (/ (* remaining-amount (get percentage dist-tuple)) u10000)))
