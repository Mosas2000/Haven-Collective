...
(define-read-only (is-authorized-minter (minter principal))
  (ok (or (is-eq minter (var-get contract-owner))
...
(define-public (mint (collection-id uint) (recipient principal))
  (let ((minted (default-to u0 (map-get? collection-minted collection-id)))
...
(define-private (mint-to-recipient (recipient principal))
  (begin (unwrap-panic (contract-call? .haven-token mint recipient)) true))
