...
(define-map offers uint (list 20 {bidder: principal, amount: uint, expires-at: uint}))
...
(define-public (make-offer (token-id uint) (amount uint) (duration uint))
  (let ((current-offers (default-to (list) (map-get? offers token-id)))
        (offer-count (default-to u0 (map-get? active-offer-count token-id)))
        (new-offer {bidder: tx-sender, amount: amount, expires-at: (+ block-height duration)}))
...
(define-public (cancel-offer (token-id uint) (offer-index uint))
  (let ((current-offers (unwrap! (map-get? offers token-id) ERR-NO-OFFERS))
