;; haven-metadata
;; Stores and manages NFT metadata separately from core token contract for upgradeability
...
(define-read-only (is-authorized-updater (updater principal))
  (ok (or 
    (is-eq updater (var-get contract-owner))
...
(define-public (set-token-uri (token-id uint) (uri (string-ascii 256)))
  (let
    ...
    (asserts! (or 
      ...
      (unwrap-panic (is-authorized-updater tx-sender))
    ) ERR-NOT-AUTHORIZED)
...
(define-private (set-metadata-item (item (tuple (token-id uint) (uri (string-ascii 256)))))
  (begin
    (map-set token-metadata (get token-id item) {
      uri: (get uri item),
      updated-at: block-height
    })
    true
